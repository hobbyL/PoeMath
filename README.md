...
- ⚙️ **分组设置中心**：外观与显示、声音与交互、学习与提醒、数据与同步、关于韵算五类入口
...
设置页按五类组织，避免把即时开关、复杂配置和数据操作混在同一层级：

| 分类 | 内容 |
|------|------|
| 外观与显示 | 国风/童趣主题、跟随系统/浅色/深色、拼音显示 |
| 声音与交互 | TTS 音色与语速、语音识别、音效、触觉反馈 |
| 学习与提醒 | 每组题量、难度、每日目标、每日提醒与周报 |
| 数据与同步 | 本地 JSON 备份/恢复、WebDAV 云端同步 |
| 关于韵算 | 版本、应用更新、隐私政策、开源许可 |

JSON 备份覆盖学习数据、普通设置和活动结算幂等标记。腾讯云 SecretId/SecretKey、
WebDAV 用户名/密码只保存在设备安全存储中，不进入 JSON 或 WebDAV 备份；恢复也
不会覆盖设备上已有的这些凭据。恢复完成后，主题设置会立即刷新，本地通知会按
恢复后的开关与时间重新调度或取消；系统通知权限仍由设备管理。

语音识别设置页进入时只读取一次设备安全存储，首帧直接显示设置结构。打开页面
不会请求腾讯云、麦克风权限或加载 Sherpa-ONNX 模型，模型初始化仍在真实录音测试
或诗词跟读开始时触发。
...
当前代码基线最近一次本地质量门禁已通过 `flutter analyze` 和全部 650 项
...
| 功能 | 说明 | 页面 |
|------|------|------|
| 诗词列表 | 按年级/作者/朝代筛选、搜索 | `poem_tab_page` |
| 诗词详情 | 全文、拼音、译文、注释、赏析、名句、作者简介（折叠卡片）| `poem_detail_page` |
| 背诵模式选择 | 渐进背诵/填空测试/选择题 统一入口 | `poem_recite_mode_page` |
| 渐进式背诵 | 4 档 Slider（全文→首字→半隐→全隐）| `poem_recite_page` |
| 填空测试 | 自动挖空诗句后半，输入答案 | `poem_quiz_page` |
| 选择题 | 上句选下句 / 诗句选作者 / 诗题选朝代 | `poem_quiz_page` |
...
| 6 种出题模式 | 求结果 / 找缺失数 / 比大小 / 竖式计算 / 链式运算 / 带括号运算 |
| 儿童数字键盘 | 大按钮（≥56pt），小数点/余数省略号与提交键可同时使用 |
| 分数 LaTeX 渲染 | MathText 自动检测分数并用 `flutter_math_fork` 渲染 |
...
- ✅ 无内购或付费功能
- ✅ 跟读默认使用本地 Sherpa-ONNX；腾讯云高精度识别需家长在「声音与交互 → 语音识别设置」填写 AK/SK、通过真实录音测试并主动开启
- ✅ JSON / WebDAV 备份不包含腾讯云 AK/SK 或 WebDAV 用户名/密码，恢复不会覆盖设备安全存储中的凭据
